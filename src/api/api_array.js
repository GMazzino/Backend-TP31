...
class ApiMethods {
  ...
  getProducts() {
    if (this.products.length !== 0) {
      ...
      logger.error(
        'Module: api_array.js Class: ApiMethods Method: getProducts Error: No products to show'
      );
  ...
  getProductById(id) {
    if (!isNaN(parseInt(id))) {
      ...
      logger.error(
        'Module: api_array.js Class: ApiMethods Method: getProductById Error: Bad request'
      );
  ...
  delProductById(id) {
    if (!isNaN(parseInt(id))) {
      ...
      logger.error(
        'Module: api_array.js Class: ApiMethods Method: delProductById Error: Bad request'
      );
      return { status: 400, content: { error: `Error en la petición` } };
    }
  }
  ...
  addNewProduct(product) {
    if (
      ...
      logger.error(
        'Module: api_array.js Class: ApiMethods Method: addNewProduct Error: Bad request'
      );
      return { status: 400, content: { error: `Error en la petición` } };
    }
  }
  ...
  updateProduct(id, product) {
    if (
      ...
      logger.error(
        'Module: api_array.js Class: ApiMethods Method: updateProduct Error: Bad request'
      );
      return { status: 400, content: { error: `Error en la petición.` } };
    }
  }
